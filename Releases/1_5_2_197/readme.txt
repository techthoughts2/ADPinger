...
--SERVICES
User specifies a Service display name.
Remote devices are scanned for this service and user designated action is performed:
i. Status - returns current status of service (running, stopped, etc)
ii. Restart - will stop the service, and then start it again
iii. Stop - stops the service
iv. Start - starts the service
...
1. Select an Action
-There are 7 possible actions to choose from: Ping, Reboot, Scan for file, Scan for directory, Copy File, Delete File, Services
...
--Services
---Enter the display name of the service. My interpretation of the class is that you can enter either the service name or the display name but I did most of my testing with the service display name, so my recommendation is to use that.  After entering a service display name, choose a service action. Status will return the current service status of all remote devices in the list.  Restart will perform a stop on the service, and the perform a start on the service.  Stop will perform a stop on the service.  Start will perform a start on the service.  It's important to note that I included a service monitoring feature into ADPinger.  In testing, I felt this was an important component to ensure that ADPinger was providing a true picture of what was happening with the service.  Rather than just shotgun out a bunch of commands ADPinger will monitor the service after a command is set for up to 45 seconds to see if it changes.  Some services shutdown or start right away, other services take longer.  Keep this in mind when using ADPinger to interact with services on remote devices.  If after 45 seconds ADPinger does not see a status change, it abandons that device and advises you that there was a problem.  Reboot, Start, and Stop are affected by this 45 second monitor.  Reboot can be affected by it twice!  Status is not affected by this.  I am not sure if I like the monitor at 45 seconds.  This time frame may be adjusted depending on feedback I receive.
...
-ADPinger will monitor a service for up to 45 seconds after you initiate a service change (stop, start, restart) to ensure that the service performs the action. If the service you specify is a slow starter, or slow to shutdown this can signfigantly impact the time it takes ADPinger to perform the desired service function.  Keep this in mind when dealing with slow services on large number of devices.  This 45 second time-frame may be adjusted in the future.